...
OAuth2 definiert sogenannte \textit{Grant Types}, die vom \textit{Client} über das Setzen eines oder mehrerer \textit{respond$\_$type} beim Aufruf des \textit{Authorization Endpoint} gewählt werden können. 
...
Während \textit{respond$\_$type=code} den \textit{Authorization Code Grant} initiiert, liefert \textit{respond$\_$type=token} den Access Token direkt nach Autorisierung zurück. 
...
OpenID Connect definiert ein weiteren \textit{response_type=id$\_$token}. 
