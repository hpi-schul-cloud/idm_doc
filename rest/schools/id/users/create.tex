...
\label{sec:rest:api:schools:id:users:create}
...
Die Felder im JSON Object, \reflisting{lst:code:rest:api:schools:id:users:create:request}, im HTTP-Post Body wird für das CREATE wird in \reftabllec{tab:rest:api:schools:id:users:create:obj}  beschrieben.
...
Es gilt für diesen Endpunkt das der Zugriff explizit Erlaubt sein muss, \reftabllec{tab:rest:api:schools:id:users:create:right}, ansonsten gibt der Endpunkt den HTTP-Statuscode 403 - Forbidden wieder.
...
\input{json}
\begin{lstlisting}[caption={Felder im JSON-Object einer CREATE anfrage per HTTP-POST auf der Route /api/schools/users/\$id},label={lst:code:rest:api:schools:id:users:create:request},frame=tlrb]
{
 user_id: "<STRING>",
 role: "<STRING>",
 start: "<TIMESTAMP>"
